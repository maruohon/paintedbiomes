...
public class PaintedBiomesEventHandler
{
    ...
    public void onInitBiomeGen(WorldTypeEvent.InitBiomeGens event)
    {
        if (Configs.getEffectiveMainConfig().useGenLayer)
        {
            ...
            newGens[0] = new GenLayerBiomeGeneration(event.getSeed(), newGens[0], event.getWorldType(), "");
            newGens[1] = new GenLayerBiomeIndex(event.getSeed(), newGens[1], event.getWorldType(), "");
